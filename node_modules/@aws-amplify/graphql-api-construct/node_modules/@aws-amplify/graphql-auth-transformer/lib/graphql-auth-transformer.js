    ...
    generateIAMPolicies(ctx) {
        if (this.generateIAMPolicyForAuthRole) {
            if (this.authPolicyResources.size === 0) {
                ...
                const iamAuthRoleArn = iam.Role.fromRoleArn(scope, 'auth-role-name', `arn:${cdk.Stack.of(scope).partition}:iam::${cdk.Stack.of(scope).account}:role/${authRole}`);
        ...
        if (this.generateIAMPolicyForUnauthRole) {
            ...
            const { scope } = ctx.stackManager;
            const iamUnauthRoleArn = iam.Role.fromRoleArn(scope, 'unauth-role-name', `arn:${cdk.Stack.of(scope).partition}:iam::${cdk.Stack.of(scope).account}:role/${unauthRole}`);
